...
function TabConciliacionInventarios(props) {
    const { Title } = Typography;
    const { tipo, setTipo, filtrosVentas, listaInventarios } = props;
    ...
    const [vtasAno, setVtasAno] = useState(filtrosVentas.anos_vtasdiarias);
    const [vtasMes, setVtasMes] = useState(filtrosVentas.meses_vtasdiarias);
    const [vtasDia, setVtasDia] = useState(filtrosVentas.dias_vtas);
    ...
    //console.log("VENTAS DIARIAS MES XXX: ", filtrosVentas);
    //console.log("LABEL DIAS: ", labelVentas);
    const tabsdos = [
        { name: 'Matriz conciliación inventarios', href: '#', current: entMes }
    ]
    ...
    useEffect(() => {
        ...
        let comprasmesenfirme = listaInventarios.comprasmesenfirme;
        let comprasconsignacionmes = listaInventarios.comprasconsignacionmes;
        let costoventa = listaInventarios.costoventa;
        let devolucionesmeslegalizada = listaInventarios.devolucionesmeslegalizada;
        let devolucionesmespendiente = listaInventarios.devolucionesmespendiente;
        let inventarioenfirme = listaInventarios.inventarioenfirme;

        if (selectedAno.length == 1) {
            if (opcion == 0) {

                let mesuno = 0; let mesdos = 0; let mestres = 0; let mescuatro = 0;
                let mescinco = 0; let messeis = 0; let messiete = 0; let mesocho = 0;
                let mesnueve = 0; let mesdiez = 0; let mesonce = 0; let mesdoce = 0;
                let datuno; let datdos; let dattres; let datcuatro;
                let datcinco; let datseis; let datsiete; let datocho;
                let datnueve; let datdiez; let datonce; let datdoce;

                let newDet = [];
                comprasmesenfirme && comprasmesenfirme.map((items) => {
                    if (items.ano == selectedAno[0].value && items.mes == 1) {
                        valoruno: parseInt(mesuno) + items.Valorbruto
                    } else
                        if (items.ano == selectedAno[0].value && items.mes == 2) {
                            valordos: parseInt(mesdos) + items.Valorbruto
                        } else
                            if (items.ano == selectedAno[0].value && items.mes == 3) {
                                valortres: parseInt(mestres) + items.Valorbruto
                            } else
                                if (items.ano == selectedAno[0].value && items.mes == 4) {
                                    valorcuatro: parseInt(mescuatro) + items.Valorbruto
                                } else
                                    if (items.ano == selectedAno[0].value && items.mes == 5) {
                                        valorcinco: parseInt(mescinco) + items.Valorbruto
                                    } else
                                        if (items.ano == selectedAno[0].value && items.mes == 6) {
                                            valorseis: parseInt(messeis) + items.Valorbruto
                                        } else
                                            if (items.ano == selectedAno[0].value && items.mes == 7) {
                                                valorsiete: parseInt(messiete) + items.Valorbruto
                                            } else
                                                if (items.ano == selectedAno[0].value && items.mes == 8) {
                                                    valorocho: parseInt(mesocho) + items.Valorbruto
                                                } else
                                                    if (items.ano == selectedAno[0].value && items.mes == 9) {
                                                        valornueve: parseInt(mesnueve) + items.Valorbruto
                                                    } else
                                                        if (items.ano == selectedAno[0].value && items.mes == 1) {
                                                            valordiez: parseInt(mesdiez) + items.Valorbruto
                                                        } else
                                                            if (items.ano == selectedAno[0].value && items.mes == 1) {
                                                                valoronce: parseInt(mesonce) + items.Valorbruto
                                                            } else
                                                                if (items.ano == selectedAno[0].value && items.mes == 1) {
                                                                    valordoce: parseInt(mesdoce) + items.Valorbruto
                                                                }
                ...
                newDet.push(datdos);
                newDet.push(dattres);
                newDet.push(datcuatro);
                newDet.push(datcinco);
                newDet.push(datseis);
                newDet.push(datsiete);
                newDet.push(datocho);
                newDet.push(datnueve);
                newDet.push(datdiez);
                newDet.push(datonce);
                newDet.push(datdoce);
...
export default TabConciliacionInventarios;